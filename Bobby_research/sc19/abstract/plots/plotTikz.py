...
def plotTikz(data, extraData, filename):
  ...
  rowLabels = ['communication $(\mu s)$', 'unzip $(\mu s)$', 'rhs $(\mu s)$', 'derivatives $(\mu s)$','wavelets $(\mu s)$']
  exLabels = ['Total time $(\mu s)$', 'Total dofs (zipped)','dofs per core']
  ...
  sz = shape(data)
  esz = len(extraData);
  str = '\\draw[very thin,color=gray, xstep=1, ystep=0.5] (0, ' + repr(-0.5*(sz[0]+esz+1)) + ') grid (13,-0.5);\n';
  out.write(str)
