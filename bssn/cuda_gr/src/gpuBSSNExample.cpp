...
//1 . read the parameter file.
    ...
    if(rank==1|| npes==1)
    {
        ...
        std::cout<<YLW<<"\tBSSN_RK45_TIME_BEGIN :"<<bssn::BSSN_RK45_TIME_BEGIN<<NRM<<std::endl;
        std::cout<<YLW<<"\tBSSN_RK45_TIME_END :"<<bssn::BSSN_RK45_TIME_END<<NRM<<std::endl;
...
//isActive=(rank*grainSz<globalSz);
    ...
    ot::Mesh * mesh=new ot::Mesh(balOct,1,bssn::BSSN_ELE_ORDER,comm,bssn::BSSN_DENDRO_GRAIN_SZ,bssn::BSSN_LOAD_IMB_TOL,bssn::BSSN_SPLIT_FIX);
    ...
    if(mesh->isActive())
    {
        ...
        cuda::computeRHS(varUnzipOut,(const double **)varUnzipIn,&(*(blkCudaList.begin())),blkCudaList.size());
        t2=MPI_Wtime();
        t_stat=t2-t1;
        par::Mpi_Reduce(&t_stat,t_stat_g,1,MPI_MIN,0,mesh->getMPICommunicator());
        par::Mpi_Reduce(&t_stat,t_stat_g+1,1,MPI_SUM,0,mesh->getMPICommunicator());
        ...
        if(!rank) std::cout<<GRN<<" GPU computation end "<<NRM<<std::endl;
#endif
