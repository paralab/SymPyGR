...
class GWExtract:
    ...
    def __init__(self,N,qprec,lmodes,spin,varNames):
        ...
        with open(fname,'r') as f:
            for row in f:
                row=row.strip().split()
                self.lebedev_theata.append(deg_radians*float(row[0]))
                self.lebedev_phi.append(deg_radians*float(row[1]))
                self.lebedev_w.append(deg_radians*float(row[2]))
    ...
    '''
    compute the SWSH coefficennts for a given l mode and it corresponding m modes. 
    @param[in] l : l mode
    returns [[real],[img]] after the lebedev quadrature
    '''

    def computeSpinWeightCoeff(self,l):
        ...
        with open(fname,'r') as f:
            for row in f:
                row=row.strip().split()
                lebedev_theata.append(float(row[0]))
                lebedev_phi.append(float(row[1]))
                lebedev_w.append(float(row[2]))
                
        #print(lebedev_w)
        numQuadPts=len(lebedev_theata);
        #print(numQuadPts)
        ...
        for m in range(-l,l+1):
            ...
            for q in range(0,numQuadPts):
                self.psi4_lm=self.psi4_lm + self.psi4_s2*lebedev_w[q]*self.swsh(self.spin,l,m,lebedev_theata[q],lebedev_phi[q])
                
            cReal.append(re(self.psi4_lm))
            cImg.append(im(self.psi4_lm))
            cArg.append(arg(self.psi4_lm))
            
        return [cReal,cImg,cArg]
